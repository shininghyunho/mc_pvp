...
public class PVPCommand {
                    ...
                    switch (op) {
                        ...
                        case "그만하기" -> {
                            ...
                            if(pvpRegion.getGameStatus().equals(GameStatus.PAUSED)) {
                                pvpRegion.cancelByLogout(pvpRegion.getOpponent(player));
                            }
                            // 상대가 다시하기를 요청한 경우
    ...
    private static boolean isValidInvite(Player inviter, Player invitee) {
        ...
        // 플레이어가 이미 PVP 중일 때
        if (UserManager.getInstance().get(invitee).getCurrentPVP() != null) {
    ...
    private static PVPRegion getPvpRegion(Player player) {
        ...
        if (user.getCurrentPVP() == null) return null;

        return RegionManager.getInstance().get(user.getCurrentPVP());
    }
    private static boolean isInGame(Player player) {
        return UserManager.getInstance().get(player).getCurrentPVP() != null;
    }
}
